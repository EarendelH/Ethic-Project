...
\textbf{1. TFMA (TensorFlow Model Analysis) Metrics (13 metrics): Table \ref{tab:performance_by_sex_baseline}}
...
\label{tab:performance_by_sex_baseline}
...
\textbf{2. Extended Fairness Metrics (5 metrics): Table \ref{tab:fairness_metrics_baseline}}
...
\begin{table}[h]
\centering
\caption{Base Model Extended Fairness Metrics Summary}
\label{tab:fairness_metrics_baseline}
\begin{tabular}{|l|c|c|c|}
\hline
...
We evaluate the mitigated model using the same 13 TFMA metrics (Table \ref{tab:fair_performance_by_sex}) and 5 extended fairness metrics (Table \ref{tab:fair_fairness_metrics}) as the baseline.
...
\begin{table}[h]
\centering
\caption{Fair Model Extended Fairness Metrics Summary}
...
\begin{table}[h]
\centering
\caption{Double-Win Scorecard: Fair Model vs.\ Base Model (13 Metrics)}
\label{tab:double_win_scorecard}
...
\begin{table}[h]
\centering
\caption{Extended Fairness Metric Comparison}
\label{tab:extended_fairness_comparison}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Metric} & \textbf{Base Model} & \textbf{Fair Model} & \textbf{Change} \\
\hline
...
The fairness gains are visible across the extended metrics. Predictive Parity decreases (0.1051 $\rightarrow$ 0.0971), NPV Equality becomes nearly equal (0.0335 $\rightarrow$ 0.0014), the AUC gap decreases (0.0531 $\rightarrow$ 0.0378), and Average Odds decreases (0.0620 $\rightarrow$ 0.0330). Disparate Impact also moves closer to parity, from 0.8886 to 0.9622. Across the complete evaluation set (Table \ref{tab:double_win_scorecard} and Table \ref{tab:extended_fairness_comparison}), 12 metrics improve in total: all 5 extended fairness metrics and 7 TFMA metrics.
...
\begin{enumerate}
    \item \textbf{Fairness is achievable:} Through adversarial debiasing, Equalized Odds constraints, and sample re-weighting, the model improves all five extended fairness metrics.
